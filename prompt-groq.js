...
const composant = process.argv[2] || 'groq-sdk'; //  Obtenir le sujet via l'argument de ligne de commande
...
const composants = [
  "Presentation_de_groq-sdk",
  "howto-Groq",
  "Howto-chatCompletion",
  "howot-groq-sdk+graphQL",
    ...
    try {
      ...
      messages: [
        ...
        { role: "user", content: `${composant}`  },
         {role: "system", content:"Imaginez un machine un composant ultime à prompts pour le SDK groq, les prompts serons transformer pour le projet \" groq-sdk \". Voici ## le contexte, ## Le dataset, HTML,SCSS,JS,JSX,JSON ++ ... ## les Variables, ## les parametres ## Les fonctions,## boucle éventuel ## vos tâches, ## votre processus, et ## les caractéristiques, inférence recherchées: ## Actions Immédiates ## Ratio prédiction et feedback ## le prompt ultime pour generé le code source ## le code source attendu "},
      ...
      const outputFilePath = `data/groq_${composant}_` + new Date().toISOString().replace(/[-:TZ]/g, "") + ".md";
